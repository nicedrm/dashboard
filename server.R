...
server <- function(input, output) {
  ...
  output$vbox3 <- renderValueBox({ valueBox( sprintf("%0.0f%%", sr.hp/m.hp*100),"힐링패치 ROAS", width = 2, icon = icon("credit-card"))})
  ...
  output$vbox6 <- renderValueBox({ valueBox( sprintf("%0.0f%%", sr.bp/m.bp*100),"버블퐁 ROAS", width = 2, icon = icon("thumbs-up", lib = "glyphicon"))})
  ...
  output$vbox9 <- renderValueBox({ valueBox( sprintf("%0.0f%%", sr.ss/m.ss*100),"씨솔트샴푸 ROAS", width = 2, icon = icon("list"))})
  ...
  output$vbox12 <- renderValueBox({ valueBox( sprintf("%0.0f%%", sr.ls/m.ls*100),"가벼운한잔 ROAS", width = 2, icon = icon("stethoscope"))})
  ...
  output$vbox15 <- renderValueBox({ valueBox( sprintf("%0.0f%%", sr.gd/m.gd*100),"굿바이찌든때 ROAS", width = 2, icon = icon("list"))})
  ...
  output$vbox18 <- renderValueBox({ valueBox( sprintf("%0.0f%%", sr.bt/m.bt*100),"버블토네이도 ROAS", width = 2, icon = icon("stethoscope"))})
  output$vbox19 <- renderValueBox({ valueBox( formatC(sr.rs, format = "d", big.mark = ","),"릴렉스팟 매출", width = 2, icon = icon("list"))})
